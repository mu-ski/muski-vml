...
def store_user_data(user: User, playlist_title, playlist):
    ...
    db.set(user.model_dump(exclude_defaults=True, exclude={'lib','lib_extended'}), f'users/{user.display_name}')
...
def prog_logic():
    ...
    if not user_db:
        ...
        if user.num_logins >= 2:
            cli.emit('Unfortunatley, due to limited (free) AI-servers we are using right now,' \
    ...
    if val:
        ...
        if pl_url:
            cli.emit(f"Voilà! Your playlist '{playlist_title}' is ready, it can be accessed on: {pl_url}")
        else:
